...
Function `transform()` function does all this:

```{r usetransform}
options(usedx=TRUE)   # uses dx etc in print method
transform(dx^dy+5*dx^dz, matrix(1:9,3,3))
options(usedx=FALSE)  # revert to default
```
...
o |> transform(M) |> transform(solve(M)) |> zap()
```
