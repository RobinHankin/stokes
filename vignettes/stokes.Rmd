...
Here I show how the `stokes` package furnishes functionality for
working with the exterior calculus, and provide numerical verification
of a number of theorems.  Notation follows that of Spivak, and Hubbard
and Hubbard.
...
correspond to each term's cross products of the $\phi$'s.  We first
...
implementation using the `STL map` class and is discussed in more
detail in the `mvp` package.
...
### Cross product of general tensors
...
We will calculate the cross product of two tensors `S1,S2` defined as follows:
...
as a single entity.  Here I define the elementary form $dx_i$ formally
and in the next section discuss the wedge product $\wedge$.  The
elementary form $dx_i$ is simply a map from $\mathbb{R}^n$ to
...
$dx_i\left(x_1,x_2,\ldots,x_n\right)=x_i$.  Thus, for example:
...
are linear: we may add and subtract these elementary forms:
...
In symbolic notation, `K1` is equal to $7dx_1\wedge dx_4\wedge dx_6
-2dx_3\wedge dx_4\wedge dx_5$. and `K2` is $dx_1\wedge dx_3+
...
In the above, note how the $dx_2\wedge dx_4$ terms combine [to give `2
4 = 113`] and the $dx_1\wedge dx_3$ term vanishes by cancellation.
...
We can alter the default print method with the `stokes_symbolic_print`
...
Function `transform()` function does all this but it is slow.  I am
not 100\% sure that there isn't a much more efficient way to do such a
...
dy_4\wedge dy_5$, except for some numerical roundoff error.  In this
case the extra rows have values small enough for the print method to
print an exact zero, but not sufficiently small to be eliminated by
the `spray` internals.  The `spray` package version 1.0-8 does not
have the capability of removing small entries (this will be available
in 1.0-9); but in the meantime we can do this:

```{r}
as.kform(index(o),zapsmall(coeffs(o)))
```

returning us to the original value of `o`.
...
To calculate `dphi`, or $d\phi$, we can use function `grad()`:
