...
`kinner()`.  Given two $k$-forms $\alpha,\beta$, function `kinner()`
returns an inner product $\left\langle\cdot,\cdot\right\rangle$ of
$\alpha$ and $\beta$.  If $\alpha=\alpha_1\wedge\cdots\wedge\alpha_k$
and $\beta=\beta_1\wedge\cdots\wedge\beta_k$, and we have an inner
product $\left\langle\alpha_i,\beta_j\right\rangle$ then
...
It is important to remove the `dt`, `dx`, `dy`, `dz` as created above
