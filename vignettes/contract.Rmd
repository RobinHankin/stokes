...
`contract()`.  Given a $k$-form $\phi\colon
...
$k$-form, then $\left(\phi_\mathbf{u}\right)_\mathbf{v}$ is a $k-2$
form with
...
index matrix) we can use the `lose=FALSE` argument:
...
(-1)^{l-1}\left(dx^{l}\otimes\left(dx^{i_1}\wedge\cdots\wedge\widehat{dx^{l}}\wedge\cdots\wedge
dx^{i_k}\right)\right)\left(\mathbf{e}_l,\left(\mathbf{v}_1,\ldots,\mathbf{v}_{k-1}\right)\right)\\ &=&
(-1)^{l-1}\left(dx^{i_1}\wedge\cdots\wedge\widehat{dx^{l}}\wedge\cdots\wedge
