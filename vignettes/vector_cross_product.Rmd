...
reals, with
$\left(\mathbf{u}\times\mathbf{v}\right)(\mathbf{w})=\left(I_\mathbf{u}\right)_\mathbf{v}(\mathbf{w})$,
where $I$ is the 3-volume element and subscripts refer to contraction.
This is easy to implement in the package.  Suppose we wish to take the
vector cross product of $\mathbf{u}=\left(1,4,2\right)^T$ and
