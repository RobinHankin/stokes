...
The inner product of two vectors ${\bf x}$ and ${\bf y}$ is usually
written $\left\langle\mathbf{x},\mathbf{y}\right\rangle$ or
$\mathbf{x}\cdot\mathbf{y}$ but the most general form would be
$\mathbf{x}^TM\mathbf{y}$ where $M$ is a matrix.
...
products are multilinear, that is $\left\langle\mathbf{x},a\mathbf{y}
+
b\mathbf{z}\right\rangle=a\left\langle\mathbf{x},\mathbf{y}\right\rangle
