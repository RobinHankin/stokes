...
$\left(\mathbb{R}^5\right)^4$ to $\mathbb{R}$.  Below, we willtest
...
collectively require only a single coefficient.  Verifying that
```S1``` is in fact alternating is straightforward:
...
The package provides ```kform``` objects which are inherently
alternating using a more efficient representation; they are described
using wedge products which are discussed next.
...
simple structure.  An elementary form is an object such as $dx_1\wedge
dx_3$, which is an alternating multilinear map from
...
map class of C++.  Spivak observes that $\Lambda^k(V)$ is spanned by
