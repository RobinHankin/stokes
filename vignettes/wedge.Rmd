...
We will consider the case $n=5,k=4$, so we have multilinear maps from
$\left(\mathbb{R}^5\right)^4$ to $\mathbb{R}$.  We can test algebraic
identities in R using the idiom furnished by the wedge package.
...
correspond to each terms's cross products of the $\phi$'s:
...
Addition of such $k$-forms is straightforward in R idiom but
algebraically nontrivial:
...
as a single entity.  Here I define the wedge product $\wedge$
formally.  Given two alternating forms $\omega$ and $\eta$, Spivak
