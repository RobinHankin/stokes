...
`dovs()` returns $n$ [compare `arity()`, which returns $k$].  The
function is, as seen above, very simple but its use is not entirely
straightforward in the context of `stokes` idiom.
...
observe that $e_5=\left(0,0,0,0,1\right)^T$ maps to zero in the sense
that $f(e_5,\mathbf{v})=f(\mathbf{v},e_5)=0$, for any
...
the second column with the same result).  Above we see that the
$k$-form `a`, evaluated with $e_5$ as one of its arguments returns
zero because the index matrix of `a` does not include the number 5.
