...
Above we see a kform `x` with `dovs` of 5.  Recall that object `x` is
a map from $V^2$ to $\mathbb{R}$, where vector space $V=\mathbb{R}^5$.
So, for example,
...
component", for that has already been discarded by the \code{map}
class.
