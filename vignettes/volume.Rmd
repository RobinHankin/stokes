...
unique $\omega$ is called the *volume element* ov $V$, determined by
...
volume of the parallelipiped spanned by the line segments from $0$ to
...
We will take $n=7$ as an example:
...
Now we create $w_1,\ldots,w_n$, another orthonormal set.  We verify as
follows:

```{r}
ro <- function (n){ qr.Q(qr(matrix(rnorm(n*n),n,n))) }  # random orthogonal matrix
M1 <- ro(7)
M2 <- M1[c(2,1,3,4,5,6,7),]
c(f(M1),f(M2))
```
