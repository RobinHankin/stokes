...
@hankin2022_stokes.  This function monograph discusses convenience
objects `dx`, `dy`, and `dz`, corresponding to elementary differential
forms (basis vectors $e_1$, $e_2$, $e_3$ are discussed in vignette
[ex](ex.html)).
...
<p style='margin-bottom:1em; margin-right:1em; text-align:right; font-family:Georgia'> <b>- Michael Spivak, 1965</b> <i>(Calculus on Manifolds, Perseus books).  Page 89</i>
</p></span>
</div>
...
As Spivak says, `dx`, `dy` and `dz` is a dual basis to $e_1,e_2,e_3$
and these are defined using function `e()`.  In this case it is safer
to pass `n=3` to function `e()` in order to specify that we are
working in $\mathbb{R}^3$.
