...
\shorttitle{The {\tt stokes} R package}
\shortauthors{R.K.S. Hankin}
%\begin{frontmatter}

\title [mode = title]{Introducing the {\tt stokes} package for the R programming language}

\author[1]{R.K.S. Hankin}[type=author,
                        auid=000,bioid=1,
                        orcid=0000-0001-5982-0415]


\ead{hankin.robin@gmail.com}
\ead[url]{www.stirling.ac.uk}

\affiliation[1]{organization={University of Stirling},
                city={Stirling},
...
\begin{highlights}
\item The {\tt stokes} package creates and manipulates tensors and
  alternating forms, stored using a compact array-based representation
\item Wedge products and the Hodge star operator are implemented
\item It is available on CRAN at
  \url{https://CRAN.R-project.org/package=stokes}.
\end{highlights}

\begin{keywords}
Stokes's theorem\sep R programming language\sep Symbolic computation
\end{keywords}
...
packages~\citep{hankin2022_disordR,hankin2022_spray}.
...
package, this is effected by the {\tt Alt()} function~\citep{hankin2026_stokes_Alt}:
...
Verification is straightforward~\citep{hankin2026_stokes_contract}:
...
Here I show that transformations may be inverted using matrix
inverses:
...
Above we see many rows with very small values, but not sufficiently
small to be eliminated by the {\tt spray} internals.  We can remove
the small entries with {\tt zap()}:
...
Following~\cite{hubbard2015}, we observe that
...
formalism~\citep{hankin2025_clifford,hestenes1987},
following~\cite{klausen2022}.
...
\bio{figs/IMG_9143}  % kept at AUT_hdrive/professional_photos
Robin Hankin is a mathematician specializing in computational
statistics.  His first degree was in pure and applied mathematics from
Trinity College, Cambridge.  After working in the UK's Health and
Safety Executive studying major industrial hazards, Robin returned to
Cambridge for a PhD in fluid mechanics, studying atmospheric
diffusion.  He then worked at The University of Auckland as a lecturer
in environmental science, publishing in the field of industrial risk
assessment.  After returning to Cambridge to study uncertainty in
global climate change as a senior research associate, and becoming an
associate professor at Auckland University of Technology, his current
research portfolio at Stirling involves computational methods as used
in statistics, physics, and game theory.
\endbio
