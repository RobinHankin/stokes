...
\begin{abstract}
  In this short article I introduce the {\tt stokes} package.
  Stokes's generalized integral theorem, viz $\int_{\partial
    X}\phi=\int_Xd\phi$, is demonstrated here using {\tt stokes},
  which provides functionality for working with tensors, alternating
  forms, wedge products, and related concepts from the exterior
  calculus.  The package uses {\tt spray} objects and follows {\tt
    disordR} discipline; it is available on CRAN at
  \url{https://CRAN.R-project.org/package=stokes}.
\end{abstract}

\hfill\includegraphics[width=1in]{\Sexpr{system.file("help/figures/stokes.png",package="stokes")}}
...
with the exterior calculus.  I go on to use it to give numerical
verification of a number of theorems.  Notation follows that of
\cite{spivak1965}, and \cite{hubbard2015}.
...
sparse} array, as implemented by the {\tt spray} package.  We will
consider the case $n=5,k=4$, so we have multilinear maps from
...
more detail in the {\tt mvp} and {\tt disordR} packages.
...
The R idiom for $S1\otimes S2$ would be {\tt tensorprod()}, or {\tt \%X\%}:
...
In the stokes package, this is effected by the {\tt Alt()} function:
...
Verifying that {\tt S1} is in fact alternating is straightforward:
...
different order: the stokes package uses the {\tt spray} package,
which in turn utilises the {\tt STL} map class of {\tt C++}.
...
function of position.  The package provides {\tt grad()} which, when given
