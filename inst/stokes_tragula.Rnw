...
Here I introduce the \pkg{stokes} package, written in the R computing
...
algebraic identities in R using the idiom furnished by the \pkg{stokes} package.  For our example we will define
