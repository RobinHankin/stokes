...
\title{Stokes's theorem in \proglang{R}}

%% for pretty printing and a nice hypersummary also set:
\Plainauthor{Robin K. S. Hankin}
\Plaintitle{Stokes's theorem in R}
\Shorttitle{Stokes's theorem in \proglang{R}}

%% an abstract and keywords

\Abstract{

  In this short article I introduce the \pkg{stokes} package which
  ...
  Notation and spirit follow \citeauthor{spivak1965}.  Stokes's
  generalized integral theorem, viz $\int_{\partial
    X}\phi=\int_Xd\phi$, is demonstrated here using the package; it is
  available on CRAN
  at\\ \url{https://CRAN.R-project.org/package=stokes}.

}
   
\Keywords{Stokes's theorem, exterior calculus, differential forms, \proglang{R}}
\Plainkeywords{Stokes's theorem, exterior calculus, differential forms, R}
...
%% \Month{MONTH}
 ...
 \Repository{https://github.com/RobinHankin/stokes}
 \DOI{10.5555/12345678}  %% For tragula
...
Here I introduce the \pkg{stokes} package, written in the \proglang{R} computing
language~\citep{rcore2022_tragula}, which furnishes functionality for working
with the exterior calculus.  I give numerical verification of a number
of theorems using the package.  Notation follows that
of~\citet{spivak1965}.
...
a vector space; \citeauthor{spivak1965} denotes the space of
multilinear maps as $\mathcal{J}^k(V)$.  Formally, multilinearity
means
...
\begin{equation}
S\left(v_1,\ldots,v_i+{v_i}',\ldots,v_k\right)=S\left(v_1,\ldots,v_i,\ldots,v_k\right)+
...
\citeauthor{spivak1965} observes that $\mathcal{J}^k(V)$ is spanned by
the $n^k$ products of the form
...
where $v_1,\ldots,v_n$ is a basis for $V$ and
...
The space spanned by such products has a natural representation in \proglang{R}
as an array of dimensions $n\times\cdots\times n=n^k$.  If \code{A} is
such an array, then the element \code{A[i\_1,i\_2,...,i\_k]} is the
...
sparse} array, as implemented by the \pkg{spray}
...
algebraic identities in \proglang{R} using the idiom furnished by the \pkg{stokes} package.  For our example we will define
...
have to load the \pkg{stokes} package:
...
we have
$S=1.5\phi_5\otimes\phi_1\otimes\phi_1\otimes\phi_1+2.5\phi_1\otimes\phi_1\otimes\phi_2\otimes\phi_3+3.5\phi_1\otimes\phi_3\otimes\phi_4\otimes\phi_2$.
Note that in some implementations the row order of object \code{S}
will differ from that of \code{M}; this phenomenon is due to the
underlying \proglang{C++} implementation using the \proglang{STL map}
class and is discussed in more detail in the \pkg{spray} and
\pkg{disordR}
packages~\citep{hankin2022_disordR_tragula,hankin2022_spray_tragula}.
...
we need to define three matrices \code{E1,E2,E3} corresponding to
points in $\left(\mathbb{R}^5\right)^4$ which are identical except for
one column.  In \code{E3}, this column is a linear combination of the
corresponding column in \code{E2} and \code{E3}:
...
which is applied to \code{E1, E2, E3}:
...
We will calculate the tensor product of two tensors \code{S1,S2}
defined as follows:
...
The \proglang{R} idiom for $S1\otimes S2$ as per
equation~\ref{definition_of_tensor_cross_product} would be
\code{tensorprod()}, or \code{\%X\%}:
...
from $V^k$ to $\mathbb{R}$.  \citeauthor{spivak1965} gives
...
package, this is effected by the \code{Alt()} function:
...
Verifying that \code{Alt(S1)} is in fact alternating is
...
there is a \code{1 2} term and a \code{2 1} term (the coefficients are
...
\code{kform} objects which are inherently alternating using a more
...
or even $K=dx_4\wedge dx_2\wedge dx_3 +5dx_1\wedge dx_4\wedge dx_2$
which would be a linear map from $\left(\mathbb{R}^n\right)^3$ to
$\mathbb{R}$ with

\begin{equation}
\left(
dx_4\wedge dx_2\wedge dx_3 +5dx_1\wedge dx_4\wedge dx_2
\right)\left(
\begin{pmatrix}a_1\\a_2\\ \vdots\\ a_n\end{pmatrix},
\begin{pmatrix}b_1\\b_2\\ \vdots\\ b_n\end{pmatrix},
...
Defining $K$ has ready \proglang{R} idiom in which we define a matrix whose rows
...
Function \code{as.kform()} takes each row of \code{M} and places the
elements in increasing order; the coefficient will change sign if the
permutation is odd.  Note that the order of the rows in \code{K} is
immaterial and indeed in some implementations will appear in a
different order: the stokes package uses the \pkg{spray}
package~\citep{hankin2022_spray} which in turn utilises the
\proglang{STL} map class of \proglang{C++}; the corresponding
\proglang{R} idiom conforms to \pkg{disordR}
discipline~\citep{hankin2022_disordR_tragula}.
...
Given $\omega\in\Lambda^k(V)$ and $\eta\in\Lambda^l(V)$,
\citeauthor{spivak1965} defines the wedge product
$\omega\wedge\eta\in\Lambda^{k+l}(V)$ as
...
define two $k$-forms, \code{K1} and \code{K2}:
...
In symbolic notation, \code{K1} is equal to $7dx_1\wedge dx_4\wedge
dx_6 -2dx_3\wedge dx_4\wedge dx_5$. and \code{K2} is $dx_1\wedge dx_3+
...
\citeauthor{spivak1965} observes that $\Lambda^k(V)$ is spanned by the
$n\choose k$ wedge products of the form
...
illustrated in the package by function \code{kform\_general()}.  Consider
the following idiom:
...
Object \code{Krel} is a two-form, specifically a map from
$\left(\mathbb{R}^4\right)^2$ to $\mathbb{R}$.  Observe that
\code{Krel} has ${4\choose 2}=6$ components, which do not appear in any
particular order.  Addition of such $k$-forms is straightforward in \proglang{R}
...
Above, note how the $dx_2\wedge dx_4$ terms combine [to give
  \code{2 4 = 113}] and the $dx_1\wedge dx_3$ term vanishes by
cancellation.
...
we can represent this more algebraically using the \code{as.symbolic()}
function:
...
In the above, \code{U} is a multilinear map from
$\left(\mathbb{R}^5\right)^2$ to $\mathbb{R}$.  Symbolically, \code{a}
represents the map that takes $(a,b,c,d,e)$ to $a$, \code{b} the map that
takes $(a,b,c,d,e)$ to \code{b}, and so on.  The asterisk \code{*} represents
...
We can alter the default print method with the \code{kform\_symbolic\_print}
option, which uses \code{as.symbolic()}:
...
This print option works nicely with the \code{d()} function for elementary
forms:
...
$V$ to \code{contract()} then this is interpreted as repeated
...
If we pass three columns to \code{contract()} the result is a $0$-form:
...
a formal $0$-form (which is represented in the package as a
\code{spray} object with a zero-column index matrix) we can use the
\code{lose=FALSE} argument:
...
Function \code{pullback()} does all this:
...
in \code{tests/testthat}.  Here I show that transformations may be inverted
...
Then we will transform according to matrix \code{M} and then transform
...
the \pkg{spray} internals.  We can remove the small entries with
\code{zap()}:
...
Given a $k$-form $\omega$, \citeauthor{spivak1965} defines the
differential of $\omega$ to be a $(k+1)$-form $d\omega$ as follows.
If
...
is the ordinary $i^\mathrm{th}$ partial
derivative~\citep[p25]{spivak1965}.  This definition allows one to
express the fundamental theorem of calculus in an arbitrary number of
dimensions without modification.  If
$f\colon\mathbb{R}^n\longrightarrow\mathbb{R}$ is a scalar function of
...
The package provides \code{grad()} which, when given a vector
...
We will use the \code{grad()} function to verify that, in
$\mathbb{R}^n$, a certain $(k-1)$-form has zero work function.
Following~\citet{hubbard2015}, we observe that
...
(where a hat indicates the absence of a term).  \citeauthor{hubbard2015} 
...
Then we can use the \code{grad()} function to calculate $d\omega$,
...
scalar functions \code{f1(), f2(), f3()}, all  $0$-forms:
...
We could use slightly slicker \proglang{R} idiom by defining elementary forms
\code{e1,e2,e3} and then defining \code{phi} to be a linear sum, weighted with
$0$-forms given by the (scalar) functions \code{f1,f2,f3}:
...
Now to evaluate first derivatives of \code{f1()} etc at point
$(1,2,3,4)$, using \code{Deriv()} from the \pkg{Deriv} package:
...
So \code{Df1} etc are numeric vectors of length 4, for example:
...
To calculate \code{dphi}, or $d\phi$, we can use function \code{grad()}:
...
Recall that \code{phi} is a function that maps $\mathbb{R}^9$ to
8-forms.  Here we choose $\left(1,2,\ldots,9\right)\in\mathbb{R}^9$
and \code{phi(1:9)} as shown above is the resulting 8-form.  Thus, if
we write $\phi_{1:9}$ for \code{phi(1:9)} we would have
...
Observe that \code{dphi(1:9)} is a 9-form, with
$d\phi_{1:9}\colon\left(\mathbb{R}^9\right)^9\longrightarrow\mathbb{R}$.
Now consider \citeauthor{spivak1965}'s theorem 4.6 (page 82), which in
this context states that a 9-form is proportional to the determinant
of the $9\times 9$ matrix formed from its arguments, with constant of
...
The \pkg{stokes} package furnishes functionality for working with
...
formalism~\citep{hankin2022_clifford_tragula,hestenes1987},
following~\citet{klausen2022}.
